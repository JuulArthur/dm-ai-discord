...
//TODO: https://github.com/sergmister/Discord-VC-Bot
//TODO: Go to branch "another-client" for most stable version
...
export async function createListeningStream({connection, interaction}) {
    ...
    receiver.speaking.on('start', async (user, speaking) => {
        ...
        //const filename = `./recordings/${Date.now()}-${getDisplayName(12, user)}.pcm`;
        const filename = `./recordings/${Date.now()}-test1.pcm`;
        const out = createWriteStream(filename, {encoding: 'binary'});
